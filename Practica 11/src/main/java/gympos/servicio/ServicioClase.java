...
public class ServicioClase {
    ...
    public void Agregar(String nombre, String instructor, LocalDateTime FechaHora, int capacidad)
            throws GymPOSException {
    ...
    public void Inscribir(int IdClase, int IdCliente) throws GymPOSException {
        ClaseGrupal c = clases.stream().filter(cl -> cl.GetId() == IdClase)
                .findFirst().orElseThrow(() -> new GymPOSException("Clase no encontrada."));
    ...
    public void Eliminar(int id) throws GymPOSException {
        ClaseGrupal c = clases.stream().filter(cl -> cl.GetId() == id)
                .findFirst().orElseThrow(() -> new GymPOSException("Clase no encontrada."));
        clases.remove(c);
        Serializador.Guardar(clases, ARCHIVO);
    }
